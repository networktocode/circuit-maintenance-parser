...
def run_command(context, exec_cmd, port=None):
    ...
    """
    if is_truthy(context.circuit_maintenance_parser.local):
        print(f"LOCAL - Running command {exec_cmd}")
        result = context.run(exec_cmd, pty=True)
    else:
        print(
            f"DOCKER - Running command: {exec_cmd} container: {context.circuit_maintenance_parser.image_name}:{context.circuit_maintenance_parser.image_ver}"
        )
        if port:
            result = context.run(
...
def build(context, cache=True, force_rm=False, hide=False):
    ...
    if result.exited != 0:
        print(
            f"Failed to build image {context.circuit_maintenance_parser.image_name}:{context.circuit_maintenance_parser.image_ver}\nError: {result.stderr}"
        )
...
def cli(context):
    ...
    Args:
        context (obj): Used to run specific commands
    """
    dev = f"docker run -it -v {context.circuit_maintenance_parser.pwd}:/local {context.circuit_maintenance_parser.image_name}:{context.circuit_maintenance_parser.image_ver} /bin/bash"
