...
1. `Recommended` All services are spun up using Docker and a local mount so you can develop locally, but circuit-maintenance-parser is spun up within the Docker container.
...
```
  ...
  clean                     Remove stopped containers that source for image `circuit-maintenance-parser:`
